...
class Renderer : public NodeVisitor {
  ...
  nlohmann::json* current_loop_data = &additional_data["loop"];

  std::vector<std::shared_ptr<nlohmann::json>> data_tmp_stack;
  std::stack<const nlohmann::json*> data_eval_stack;
  std::stack<const DataNode*> not_found_stack;

  bool break_rendering {false};

  static bool truthy(const nlohmann::json* data) {
    if (data->is_boolean()) {
  ...
  void print_data(const std::shared_ptr<nlohmann::json>& value) {
    if (value->is_string()) {
      if (config.html_autoescape) {
        *output_stream << htmlescape(value->get_ref<const nlohmann::json::string_t&>());
      } else {
        *output_stream << value->get_ref<const nlohmann::json::string_t&>();
      }
    } else if (value->is_number_unsigned()) {
      *output_stream << value->get<const nlohmann::json::number_unsigned_t>();
    } else if (value->is_number_integer()) {
      *output_stream << value->get<const nlohmann::json::number_integer_t>();
  ...
  const std::shared_ptr<nlohmann::json> eval_expression_list(const ExpressionListNode& expression_list) {
  ...
  void make_result(const nlohmann::json&& result) {
    auto result_ptr = std::make_shared<nlohmann::json>(result);
    data_tmp_stack.push_back(result_ptr);
    data_eval_stack.push(result_ptr.get());
  }

  template <size_t N, size_t N_start = 0, bool throw_not_found = true> std::array<const nlohmann::json*, N> get_arguments(const FunctionNode& node) {
    ...
    std::array<const nlohmann::json*, N> result;
  ...
  void visit(const DataNode& node) {
    if (additional_data.contains(node.ptr)) {
      ...
      if (function_data.operation == FunctionStorage::Operation::Callback) {
        ...
        const auto value = std::make_shared<nlohmann::json>(function_data.callback(empty_args));
  ...
  void visit(const FunctionNode& node) {
    ...
    case Op::Add: {
      ...
      if (args[0]->is_string() && args[1]->is_string()) {
        make_result(args[0]->get_ref<const nlohmann::json::string_t&>() + args[1]->get_ref<const nlohmann::json::string_t&>());
      } else if (args[0]->is_number_integer() && args[1]->is_number_integer()) {
        make_result(args[0]->get<const nlohmann::json::number_integer_t>() + args[1]->get<const nlohmann::json::number_integer_t>());
      } else {
        make_result(args[0]->get<const nlohmann::json::number_float_t>() + args[1]->get<const nlohmann::json::number_float_t>());
      }
    } break;
    case Op::Subtract: {
      const auto args = get_arguments<2>(node);
    ...
    case Op::Division: {
      const auto args = get_arguments<2>(node);
      if (args[1]->get<const nlohmann::json::number_float_t>() == 0) {
    ...
    case Op::Power: {
      const auto args = get_arguments<2>(node);
      if (args[0]->is_number_integer() && args[1]->get<const nlohmann::json::number_integer_t>() >= 0) {
        const auto result = static_cast<nlohmann::json::number_integer_t>(std::pow(args[0]->get<const nlohmann::json::number_integer_t>(), args[1]->get<const nlohmann::json::number_integer_t>()));
        make_result(result);
      } else {
        const auto result = std::pow(args[0]->get<const nlohmann::json::number_float_t>(), args[1]->get<const nlohmann::json::number_integer_t>());
        make_result(result);
      }
    } break;
    case Op::Modulo: {
    ...
    case Op::Capitalize: {
      auto result = get_arguments<1>(node)[0]->get<nlohmann::json::string_t>();
    ...
    case Op::DivisibleBy: {
      const auto args = get_arguments<2>(node);
      const auto divisor = args[1]->get<const nlohmann::json::number_integer_t>();
      make_result((divisor != 0) && (args[0]->get<const nlohmann::json::number_integer_t>() % divisor == 0));
    } break;
    case Op::Even: {
      make_result(get_arguments<1>(node)[0]->get<const nlohmann::json::number_integer_t>() % 2 == 0);
    } break;
    case Op::Exists: {
      auto&& name = get_arguments<1>(node)[0]->get_ref<const nlohmann::json::string_t&>();
      make_result(data_input->contains(nlohmann::json::json_pointer(DataNode::convert_dot_to_ptr(name))));
    } break;
    case Op::ExistsInObject: {
      const auto args = get_arguments<2>(node);
      auto&& name = args[1]->get_ref<const nlohmann::json::string_t&>();
    ...
    case Op::Float: {
      make_result(std::stod(get_arguments<1>(node)[0]->get_ref<const nlohmann::json::string_t&>()));
    } break;
    case Op::Int: {
      make_result(std::stoi(get_arguments<1>(node)[0]->get_ref<const nlohmann::json::string_t&>()));
    } break;
    case Op::Last: {
    ...
    case Op::Length: {
      ...
      if (val->is_string()) {
        make_result(val->get_ref<const nlohmann::json::string_t&>().length());
      } else {
    ...
    case Op::Range: {
      std::vector<int> result(get_arguments<1>(node)[0]->get<const nlohmann::json::number_integer_t>());
      std::iota(result.begin(), result.end(), 0);
      make_result(std::move(result));
    } break;
    case Op::Round: {
      const auto args = get_arguments<2>(node);
      const auto precision = args[1]->get<const nlohmann::json::number_integer_t>();
      const double result = std::round(args[0]->get<const nlohmann::json::number_float_t>() * std::pow(10.0, precision)) / std::pow(10.0, precision);
      if (precision == 0) {
    ...
    case Op::Sort: {
      auto result_ptr = std::make_shared<nlohmann::json>(get_arguments<1>(node)[0]->get<std::vector<nlohmann::json>>());
    ...
    case Op::Join: {
      const auto args = get_arguments<2>(node);
      const auto separator = args[1]->get<nlohmann::json::string_t>();
  ...
  void visit(const SetStatementNode& node) {
    ...
    ptr = "/" + ptr;
    additional_data[nlohmann::json::json_pointer(ptr)] = *eval_expression_list(node.expression);
  }
...
public:
  ...
  void render_to(std::ostream& os, const Template& tmpl, const nlohmann::json& data, nlohmann::json* loop_data = nullptr) {
    output_stream = &os;
